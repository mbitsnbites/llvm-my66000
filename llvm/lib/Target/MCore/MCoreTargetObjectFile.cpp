//===-- MCoreTargetObjectFile.cpp - MCore object files --------------------===//
//
...
void MCoreTargetObjectFile::Initialize(MCContext &Ctx, const TargetMachine &TM){
...
static unsigned getMCoreSectionType(SectionKind K) {
  if (K.isBSS())
    return ELF::SHT_NOBITS;
  return ELF::SHT_PROGBITS;
}

static unsigned getMCoreSectionFlags(SectionKind K, bool IsCPRel) {
  unsigned Flags = 0;
...
MCSection *MCoreTargetObjectFile::getExplicitSectionGlobal(
  ...
  return getContext().getELFSection(SectionName, getMCoreSectionType(Kind),
                                    getMCoreSectionFlags(Kind, IsCPRel));
}

MCSection *MCoreTargetObjectFile::SelectSectionForGlobal(
    const GlobalObject *GO, SectionKind Kind, const TargetMachine &TM) const {
...
MCSection *MCoreTargetObjectFile::getSectionForConstant(const DataLayout &DL,
                                                        SectionKind Kind,
                                                        const Constant *C,
