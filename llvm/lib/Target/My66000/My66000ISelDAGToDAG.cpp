  ...
  if (N->getOpcode() == ISD::AND) {
    if (isOpcWithIntImmediate(N, ISD::AND, Andimm)) {
      ...
      if (isOpcWithIntImmediate(N->getOperand(0).getNode(), ISD::SRL, Shfimm)) {
        ...
        unsigned Offset = Shfimm+1;
//dbgs() << "\tunsigned extract pattern #1: w=" << Width << " o=" << Offset << "\n";
...
//dbgs() << "\tunsigned extract pattern #2: w=" << Width <<  "\n";
...
//dbgs() << "\tsigned extract pattern #3: w=" << Shf2imm <<  " o=" << Shfimm << "\n";
      return true;
    }
  } else if (N->getOpcode() == ISD::SRL) {
//dbgs() << "\tSRL extract not implemented\n";
}
  return false;
}
